...
function [] = parsingMNIST()
    ...
    for i = 1:numel(file_name)
        crt_file = fullfile(dataset_path, file_name{i});
        fprintf('Parsing %s\n', crt_file);
        fp = fopen(crt_file, 'r');
        magic = fread(fp, 1, 'int32', 0, 'ieee-be');        
        if magic == 2051 % image file
            numImages = fread(fp, 1, 'int32', 0, 'ieee-be'); % the num of images
            numRows = fread(fp, 1, 'int32', 0, 'ieee-be'); % the size of images
            numCols = fread(fp, 1, 'int32', 0, 'ieee-be'); % the size of images
